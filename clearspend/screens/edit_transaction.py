"""Edit Transaction screen - pre-filled form to modify an existing transaction."""
...
class EditTransactionScreen(Screen):
    ...
    def load_transaction(self, txn: dict):
        """Pre-fill all fields from an existing transaction dict."""
        self.txn_id           = txn.get("id", 0)
        self.tx_type          = txn.get("type", "expense")
        self.selected_category = txn.get("category", "")
        self.selected_date    = txn.get("date", date.today().isoformat())

        # Wait until widgets are bound before setting text fields
        from kivy.clock import Clock
        def _fill(*_):
            self.ids.amount_field.text   = str(txn.get("amount", ""))
            self.ids.desc_field.text     = txn.get("description", "") or ""
            self.ids.category_field.text = self.selected_category
