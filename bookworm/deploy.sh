...
sudo tailscale funnel --bg --set-path /bookworm http://127.0.0.1:$PORT 2>/dev/null || \
...
for i in $(seq 1 30); do
  if curl -sf http://127.0.0.1:$PORT/api/health > /dev/null 2>&1; then
    echo ""
    echo "=== DEPLOYED ==="
    echo "Local:    http://localhost:$PORT/bookworm/"
    echo "Tailnet:  https://supersecretnas.tailc3a431.ts.net/bookworm/"
    exit 0
  fi
  printf "."
  sleep 2
